...
def import_to_data(file_url, field_data, m2m_fields=None):
    ...
    for i, row in enumerate(range(table.max_row)):
        ...
        for index, key in enumerate(field_data.keys()):
            ...
            # Due to the fact that after importing numerical types in Excel, there will be a problem of adding numbers to 0,
            # so please proceed with the processing
