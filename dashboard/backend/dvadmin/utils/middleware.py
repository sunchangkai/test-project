...
class ApiLoggingMiddleware(MiddlewareMixin):
    """
    Middleware for store API access logs
    """
    ...
    def __handle_response(self, request, response):
        # request_data,request_ip added by PermissionInterfaceMiddleware
        body = getattr(request, "request_data", {})
        # if the request contains a password, replace it with *
        # (temporarily use the password request parameter for all interfaces first)
        ...
        operation_log, create = OperationLog.objects.update_or_create(
