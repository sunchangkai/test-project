...
class Command(BaseCommand):
    """
    init menu: python3 manage.py generate_init_json
    eg：
    generate all：python3 manage.py generate_init_json
    generate a model： python3 manage.py generate_init_json users
    """
    ...
    def serializer_data(self, serializer, query_set: QuerySet):
        ...
        with open(
            os.path.join(BASE_DIR, f"init_{query_set.model._meta.model_name}.json"), "w"
        ) as f:
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="*",
            type=str,
            help="Initialize the generated table name",
        )
    ...
    def generate_dept(self):
        self.serializer_data(
            DeptInitSerializer, Dept.objects.filter(parent_id__isnull=True)
        )

    def generate_menu(self):
        self.serializer_data(
            MenuInitSerializer, Menu.objects.filter(parent_id__isnull=True)
        )
    ...
    def generate_dictionary(self):
        self.serializer_data(
            DictionaryInitSerializer, Dictionary.objects.filter(parent_id__isnull=True)
        )

    def generate_system_config(self):
        self.serializer_data(
            SystemConfigInitSerializer,
            SystemConfig.objects.filter(parent_id__isnull=True),
        )
    ...
    def handle(self, *args, **options):
        ...
        for generate_name in generate_name:
            if generate_name not in generate_name_dict:
                print(
                    f"The initialization method has not been configured yet\n{generate_name_dict}"
                )
                raise Exception(
                    f"The initialization method has not been configured yet, Configured items:{list(generate_name_dict.keys())}"
                )
