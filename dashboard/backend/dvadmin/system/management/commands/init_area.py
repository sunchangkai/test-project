...
def area_list(code_list, pcode=None, depth=1):
    ...
    for code_dict in code_list:
        ...
        pinyin = "".join(
            ["".join(i) for i in pypinyin.pinyin(name, style=pypinyin.NORMAL)]
        )
...
def main():
    with open(
        os.path.join(BASE_DIR, "dvadmin", "system", "util", "pca-code.json"),
        "r",
        encoding="utf-8",
    ) as load_f:
...
class Command(BaseCommand):
    """
    init command: python manage.py init
    """
        ...
        if dispatch.is_tenants_mode():
            ...
            for tenant in get_tenant_model().objects.exclude(schema_name="public"):
                with tenant_context(tenant):
                    print(f"tenant[{connection.tenant.schema_name}]start init data...")
                    main()
                    print(f"tenant[{connection.tenant.schema_name}]finish init data！")
        else:
            main()
