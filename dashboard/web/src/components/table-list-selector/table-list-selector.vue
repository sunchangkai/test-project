<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%" height="280" align="center" size="small">
      ...
      <template v-for="(item, index) in gridOptions.columns">
        ...
        <el-table-column v-else :key="index" :prop="item.field" :label="item.title" width="120">
        </el-table-column>
      </template>
      <el-table-column
        label="Operation"
      ...
      >
        <template slot-scope="scopes">
          <el-button
            style="padding: 0"
            :disabled="item.disabled ? item.disabled({ ...scopes, tableData }) : false"
            type="text"
            size="small"
            :circle="item.circle ? item.circle : false"
            v-for="(item, index) in colButtons.btns"
            :icon="item.icon ? item.icon : ''"
            :key="index"
            @click="item.click({ ...scopes, tableData })"
          >
            {{ item.text }}
          </el-button>
        </template>
  ...
  computed: {
    _elProps() {
      ...
      if (this.multiple) {
        ...
        defaultElProps.radioConfig = this.elProps ? this.elProps.radioConfig : {};
        defaultElProps.columns = [
          {
            type: 'radio',
            width: 60,
    ...
    openDialog() {
      ...
      setTimeout(() => {
        if (that._options.length > 0) {
          that._options.map(item => item[that._elProps.props.value]);
          // ids.forEach((id) => {
          //   console.log(111, id)
