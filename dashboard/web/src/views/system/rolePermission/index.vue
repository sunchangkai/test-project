...
<template>
  <div>
    <div style="margin: 10px">
      <el-button type="primary" size="mini" @click="submitPermisson" v-permission="'Save'"
        >Save
      </el-button>
    </div>
  ...
  methods: {
    ...
    getDeptData() {
      deptApi.GetListAll().then(ret => {
        this.deptOptions = XEUtils.toArrayTree(ret.data, {
          parentKey: 'parent',
          strict: false,
        });
      });
    ...
    handleCheckClick(data, checked) {
      const { menuPermission, children, parent } = data;
