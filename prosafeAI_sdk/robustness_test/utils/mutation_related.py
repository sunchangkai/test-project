...
class Mutators(object):
    def __init__(self, params, model: MyModel):
        """
...
class PixelModification(object):
    ...
    def gaussian_blur(self, img, params, seed):
        ...
        :return:
        """
        kernel_size = params["gaussian_blur"][
            "kernel_size"
        ]  # kernel size must smaller than the minimal edge length of the images, and it should be odd number
        if kernel_size % 2 != 1:
            kernel_size = kernel_size - 1
    ...
    def FGSM(self, img, params, seed):
        ...
        grads = self.model.get_gradient(data=img, y_true=y_true)
